...
class WorkoutPlanner:
    ...
    async def _generate_exercise_duration(self, exercise: Exercise, section_type: str) -> str:
        """Generate appropriate duration for individual exercise using AI"""
        try:
            prompt = f"""
            ...
            duration = duration_response.strip().split('\n')[0].strip()
            return duration if duration else "2 minutes"
            
        except Exception as e:
            logger.error(f"Error generating exercise duration: {str(e)}")
            # Return default duration based on section type
